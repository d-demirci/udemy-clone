...
class UserLoginForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.user = None
        self.fields['email'].widget.attrs.update({'placeholder': 'e-posta adresiniz'})
...
class ProfileUpdateForm(forms.ModelForm):
    # def __init__(self, *args, **kwargs):
    #     super().__init__(*args, **kwargs)
    #     self.fields["avatar"].widget.attrs.update({'class': 'form-group'})
    #     self.fields["last_name"].widget.attrs.update({'class': 'form-group'})

    avatar = forms.ImageField(widget=forms.FileInput(attrs={'class': 'form-control-file'}))
    bio = forms.CharField(widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 10}))
